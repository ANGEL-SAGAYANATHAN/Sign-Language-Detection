import 'dart:async';
import 'dart:io';

import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:flutter_tflite/flutter_tflite.dart';
import 'package:image_picker/image_picker.dart';
import 'package:csv/csv.dart';
import 'package:camera/camera.dart';

void main() {
  runApp(MyApp());
}

class MyApp extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Plant Classifier',
      theme: ThemeData(
        primarySwatch: Colors.blue,
      ),
      home: MyHomePage(),
    );
  }
}

class MyHomePage extends StatefulWidget {
  @override
  _MyHomePageState createState() => _MyHomePageState();
}

class _MyHomePageState extends State<MyHomePage> {
  File? _image;
  List<dynamic>? _output;
  bool _loading = false;

  late CameraController _cameraController;
  late CameraImage _savedImage;
  bool _isLiveCapture = false;
  bool _isBackCamera = true;

  @override
  void initState() {
    super.initState();
    _loading = true;
    loadModel().then((value) {
      setState(() {
        _loading = false;
      });
    });

    availableCameras().then((cameras) {
      if (cameras.length > 0) {
        _initializeCamera(cameras.first);
      }
    });
  }

  Future<void> _initializeCamera(CameraDescription camera) async {
    _cameraController = CameraController(camera, ResolutionPreset.medium);
    await _cameraController.initialize();
    if (mounted) {
      setState(() {});
    }
  }

  Future<void> loadModel() async {
    Tflite.close();
    try {
      String modelFile = 'assets/model.tflite';
      String labelsFile = 'assets/labels.txt';
      await Tflite.loadModel(
        model: modelFile,
        labels: labelsFile,
      );
    } on PlatformException {
      print('Failed to load model.');
    }
  }

  Future<void> _pickImage(ImageSource source) async {
    final image = await ImagePicker().getImage(source: source);
    if (image == null) return;

    setState(() {
      _image = File(image.path);
      _isLiveCapture = false;
    });
    classifyImage();
  }

  Future<List<String>> getPrecautions(String label) async {
    String csvString = await rootBundle.loadString('assets/signcode.csv');
    List<List<dynamic>> csvTable = CsvToListConverter().convert(csvString);
    for (List<dynamic> row in csvTable) {
      if (row.isNotEmpty && row[0].toString() == label) {
        return List<String>.from(row.sublist(1));
      }
    }
    return [];
  }

  Future<void> showPredictionDetails(String label) async {
    List<String> precautions = await getPrecautions(label);

    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Prediction Details'),
          content: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Prediction: $label', style: TextStyle(fontSize: 18)),
              SizedBox(height: 10),
              Text('Sign Word:', style: TextStyle(fontWeight: FontWeight.bold)),
              for (String precaution in precautions) Text('- $precaution'),
            ],
          ),
          actions: [
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).pop();
              },
              child: Text('Close'),
            ),
          ],
        );
      },
    );
  }

  Future<void> classifyImage() async {
    if (_image == null) return;

    setState(() {
      _loading = true;
    });

    try {
      var output = await Tflite.runModelOnImage(
        path: _image!.path,
        numResults: 5,
        threshold: 0.2,
        imageMean: 127.5,
        imageStd: 127.5,
      );
      setState(() {
        _loading = false;
        _output = output;
      });

      if (_output != null) {
        showPredictionDetails(_output![0]['label']);
      }
    } on PlatformException {
      print('Failed to classify image.');
    }
  }

  Future<void> classifyLive() async {
    if (!_isLiveCapture) return;

    try {
      if (_cameraController != null && _cameraController.value.isInitialized) {
        XFile file = await _cameraController.takePicture();
        if (file != null) {
          var output = await Tflite.runModelOnImage(
            path: file.path,
            numResults: 5,
            threshold: 0.2,
            imageMean: 127.5,
            imageStd: 127.5,
          );
          setState(() {
            _output = output;
          });

          if (_output != null) {
            // You can handle live predictions here
            print('Live Prediction: ${_output![0]['label']}');
          }
        }
      }
    } on PlatformException {
      print('Failed to classify live image.');
    }
  }

  Future<void> _switchCamera() async {
    CameraDescription newCamera;

    if (_isBackCamera) {
      newCamera = await _getFrontCamera();
    } else {
      newCamera = await _getBackCamera();
    }

    await _cameraController.dispose();
    _initializeCamera(newCamera);

    setState(() {
      _isBackCamera = !_isBackCamera;
    });
  }

  Future<CameraDescription> _getBackCamera() async {
    List<CameraDescription> cameras = await availableCameras();
    for (CameraDescription camera in cameras) {
      if (camera.lensDirection == CameraLensDirection.back) {
        return camera;
      }
    }
    return cameras.first;
  }

  Future<CameraDescription> _getFrontCamera() async {
    List<CameraDescription> cameras = await availableCameras();
    for (CameraDescription camera in cameras) {
      if (camera.lensDirection == CameraLensDirection.front) {
        return camera;
      }
    }
    return cameras.first;
  }

  @override
  void dispose() {
    Tflite.close();
    _cameraController.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Plant Classifier'),
      ),
      body: _loading
          ? Center(child: CircularProgressIndicator())
          : Padding(
        padding: const EdgeInsets.all(16.0),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            _isLiveCapture
                ? Expanded(
              child: _cameraController.value.isInitialized
                  ? CameraPreview(_cameraController)
                  : Container(),
            )
                : _image == null
                ? Expanded(
              child: Center(
                child: Text('No image selected.'),
              ),
            )
                : Expanded(
              child: Image.file(
                _image!,
                fit: BoxFit.cover,
              ),
            ),
            SizedBox(height: 20),
            ElevatedButton(
              onPressed: () => _pickImage(ImageSource.gallery),
              child: Text('Pick Image from Gallery'),
            ),
            SizedBox(height: 10),
            ElevatedButton(
              onPressed: _switchCamera,
              child: Text('Switch Camera'),
            ),
            SizedBox(height: 10),
            ElevatedButton(
              onPressed: () async {
                setState(() {
                  _isLiveCapture = true;
                });
                if (_cameraController != null && _cameraController.value.isInitialized) {
                  await _cameraController.startImageStream((CameraImage image) {
                    if (_isLiveCapture) {
                      _savedImage = image;
                      classifyLive();
                    }
                  });
                }
              },
              child: Text('Live Capture'),
            ),
            SizedBox(height: 20),
            ElevatedButton(
              onPressed: () async {
                setState(() {
                  _isLiveCapture = false;
                });
                if (_cameraController != null && _cameraController.value.isInitialized) {
                  await _cameraController.stopImageStream();
                }
                classifyImage();
              },
              child: Text('Predict'),
            ),
            SizedBox(height: 20),
            _output != null
                ? FutureBuilder<List<String>>(
              // Fetch precautions asynchronously based on the predicted label
              future: getPrecautions(_output![0]['label']),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return CircularProgressIndicator();
                } else if (snapshot.hasError) {
                  return Text('Error loading precautions');
                } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
                  return Text('No precautions found for the predicted label');
                } else {
                  // Display the corresponding sign word
                  return Text(
                    'Sign Word: ${snapshot.data![0]}',
                    style: TextStyle(fontSize: 18),
                  );
                }
              },
            )
                : Container(),
          ],
        ),
      ),
    );
  }
}
